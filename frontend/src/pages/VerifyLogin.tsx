...
const VerifyLogin: React.FC = () => {
    ...
    const { mutate: verifyOtp, isPending: isVerifyingOtp } = api.auth.verifyLoginApiV1AuthVerifyLoginPost.useMutation(
        undefined,
        {
            ...
            onError: (err) => {
                const msg = getErrorMessage(err, "Verification failed. Please check the code and try again.");
                toast.error(msg);
            }
        }
    );
                                ...
                                <CardContent className="space-y-6 flex flex-col items-center justify-center">
                                    {isVerifyingToken ? (
                                        <div className="text-center space-y-4 flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-sm text-foreground/70 text-center">Checking your verification link...</p>
                                        </div>
                                    ) : error ? (
                                        <div className="text-center space-y-4 flex flex-col items-center justify-center">
                                        ...
                                        <div className="text-center space-y-4 flex flex-col items-center justify-center">
                                            <Loader2 className="w-12 h-12 text-primary animate-spin mx-auto" />
                                            <p className="text-sm text-foreground/70 text-center">Preparing verification...</p>
                                        </div>
                                    ) : (
                                        <>
                                            <p className="mt-0 text-sm text-foreground/70 text-center">Please enter the 6-digit code sent to your email to complete verification.</p>
